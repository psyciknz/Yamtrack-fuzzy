...
def update_episode_images(apps, schema_editor):
    ...
    episode_items = Item.objects.filter(
        media_type=MediaTypes.EPISODE.value, image=settings.IMG_NONE
    )
    ...
    logger.info(
        "Found %s episodes with default images to process", episode_items.count()
    )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            update_episode_images, reverse_code=migrations.RunPython.noop
        ),
    ]
